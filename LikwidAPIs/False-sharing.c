...
int main(int argc, char* argv[])
{
    ...
    int gid;
    double result = 0.0;
    char estr[] = "MEM_LOAD_L3_HIT_RETIRED_XSNP_HITM:PMC0,MEM_INST_RETIRED_ALL:PMC2";
    char* enames[2] ={"MEM_LOAD_L3_HIT_RETIRED_XSNP_HITM:PMC0","MEM_INST_RETIRED_ALL:PMC2"};
	...
	for(i = 0 ; i< topo->numHWThreads; i++){
		float pmco = perfmon_getLastResult(gid, 0, i);
		float pmc1 = perfmon_getLastResult(gid, 1, i);
		printf("- Local LLC FALSE SHARING RATE at CPU %d: %f\n",i,pmco/pmc1);
	}
	printf("\n");
	...
	for(int i = 0 ; i< topo->numHWThreads ; i++){
		float pmco = perfmon_getLastResult(gid, 0 , i);
		float pmc2 = perfmon_getLastResult(gid, 1 , i);
		printf("- Local LLC FALSE SHARING RATE at CPU %d: %f\n",i,pmco/pmc2);
